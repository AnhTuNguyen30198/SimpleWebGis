...
function getGeoCMRRoadsToAjax($paPDO,$paSRID,$paPoint)
{
    ...
    $strMinDistance = "SELECT min(ST_Distance('".$paPoint."',ST_AsText(geom))) from gadm36_vnm_2";
    $mySQLStr = "SELECT ST_AsGeoJson(geom) as geo from gadm36_vnm_2 where ".$strDistance." = (".$strMinDistance.") and ".$strDistance." < 0.05";
    //echo $mySQLStr;
    //echo "<br><br>";
...
function getInfoCMRRoadsToAjax($paPDO,$paSRID,$paPoint)
{
    ...
    $strDistance = "ST_Distance('".$paPoint."',ST_AsText(geom))";
    $strMinDistance = "SELECT min(ST_Distance('".$paPoint."',ST_AsText(geom))) from gadm36_vnm_2";
    $mySQLStr = "SELECT gid, ST_Length(geom) as leng from gadm36_vnm_2 where ".$strDistance." = (".$strMinDistance.") and ".$strDistance." < 0.05";
    //echo $mySQLStr;
    //echo "<br><br>";
    $result = query($paPDO, $mySQLStr);
