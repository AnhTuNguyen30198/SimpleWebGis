...
function getGeoEagleToAjax($paPDO,$paSRID,$paPoint)
{
    ...
    $strMinDistance = "SELECT min(ST_Distance('".$paPoint."',ST_AsText(geom))) from gadm36_vnm_1";
    $mySQLStr = "SELECT ST_AsGeoJson(geom) as geo from gadm36_vnm_1 where ".$strDistance." = (".$strMinDistance.") and ".$strDistance." < 0.05";
    //echo $mySQLStr;
    //echo "<br><br>";
...
function getInfoEagleToAjax($paPDO,$paSRID,$paPoint)
{
    ...
    $strDistance = "ST_Distance('".$paPoint."',ST_AsText(geom))";
    $strMinDistance = "SELECT min(ST_Distance('".$paPoint."',ST_AsText(geom))) from gadm36_vnm_1";
    $mySQLStr = "SELECT gid from gadm36_vnm_1 where ".$strDistance." = (".$strMinDistance.") and ".$strDistance." < 0.05";
    //echo $mySQLStr;
    //echo "<br><br>";
    $result = query($paPDO, $mySQLStr);
